...
- (void)buildGrid {
  ...
  for (int x = 0; x < _gridFrame.size.height - x; x = x + gridLineDistance) {
    ...
    [linePath addCurveToPoint:CGPointMake(_gridFrame.size.width, location)
                controlPoint1:CGPointMake(10, 104 + x)
                controlPoint2:CGPointMake(152, 217 + x + (x / 2))];
...
- (void)buildOvals {
  ...
  CGRect innerRect = CGRectMake(centerXDiff, _gridFrame.origin.y,
                                _gridFrame.size.width * offset,
                                _gridFrame.size.height * offset);
  UIBezierPath *ovalPathInner =
