...
- (void)viewDidLoad {
  ...
  searchDotIncrement =
      searchStartDistanceY / [[[self selectedFriend] Accuracy] floatValue];
...
- (void)drawGridLines {
  ...
  _gridImageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(self.beaconPosition.x - (imageSizeX / 2),
                               self.beaconPosition.y - 100, imageSizeX,
                               imageSizeY)];

  [self.view addSubview:_gridImageView];

  // add grid lines layer
  BeaconGrid *beaconGrid =
      [[BeaconGrid layer] initWithFrame:[_gridImageView bounds]];
...
- (void)addBeaconHalo {
  // add image
  float imageSize = 80;
  _beaconViewMuti = [[UIImageView alloc]
      initWithFrame:CGRectMake(self.beaconPosition.x - (imageSize / 2),
                               self.beaconPosition.y - (imageSize / 2),
                               imageSize, imageSize)];
  ...
  UIImage *fbImage = [UIImage
      imageWithData:[NSData
                        dataWithContentsOfURL:
                            [NSURL
                                URLWithString:self.selectedFriend.PictureURL]]];
  ...
  flashingDot.position = CGPointMake(
      self.beaconPosition.x, self.beaconPosition.y + searchStartDistanceY);
  ...
  // you can specify the number of halos by initial method or by instance
  // property "haloLayerNumber"
  MultiplePulsingHaloLayer *multiLayer =
      [[MultiplePulsingHaloLayer alloc] initWithHaloLayerNum:3
                                            andStartInterval:1];
  ...
  [self.view.layer insertSublayer:self.mutiHalo
                            below:self.beaconViewMuti.layer];
...
// called from location manager(beacon loop in FriendsTableViewController) to
...
- (void)updateDisplay {
  if ([[[self selectedFriend] Accuracy] intValue] < 0) {
    ...
    float roundedValue =
        round(2.0f * [[[self selectedFriend] Accuracy] floatValue]) / 2.0f;
    NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
    [formatter setMaximumFractionDigits:1];
    [formatter setRoundingMode:NSNumberFormatterRoundDown];
    NSString *roundedAccuracy =
        [formatter stringFromNumber:[NSNumber numberWithFloat:roundedValue]];

    [[self accuracyLabel] setText:roundedAccuracy];

    [[self metersFeetLabel]
        setText:[self metersToFeet:[roundedAccuracy floatValue]]];
...
- (void)updateDotPosition:(NSString *)distance {
  ...
  flashingDot.position =
      CGPointMake(self.beaconPosition.x, self.beaconPosition.y + newYposition);
